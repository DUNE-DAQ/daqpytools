...
Before reading this, you really should have read [logging.md](Logging.md) docs as that contains the basic context behind _why_ these best practices are recommended, as well as how to best utilise them.
...
For context, in the native Python logging framework the highest possible logger is the (usually unnamed) root loger. For example, calling `logging.getLogger("top") will usually yield you a logger called {root}."top". Calling `logging.getLogger()` gets you the {root} logger. 

As the root logger is the highest logger which every logger inherits from, modifying this logger will have a _global_ effect on all your loggers, which is almost always undesriable. 
...
In this case, `drunc` serves as the psuedo-root logger in which no handlers are defined. All further loggers are inherited from this clean slate. 
...
Use `get_daq_loggers` to initialise it once. 

Following the previous tip, if you feel the need to get an already-initialised logger with `get_daq_loggers`, consider making a child. 

All handlers you expect to use by default should be initialised with `get_daq_loggers`. 
