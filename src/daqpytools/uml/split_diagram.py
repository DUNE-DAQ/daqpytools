#!/usr/bin/env python3
"""Split styled UML dot files into connected clusters.
...
Usage:
    python split_diagram.py <input.dot> [--output-directory DIR]
         [--format png] [--concise] [--suppress-verbose]

Example:
    python split_diagram.py pics/classes_styled.dot --output-directory pics/split
    
    # With --concise to remove type hints:
    python split_diagram.py pics/classes_styled.dot 
        --output-directory pics/split --concise
"""
...
def find_connected_components(
    node_ids: set[str], edges: Sequence[tuple[str, str, str]]
) -> list[set[str]]:
...
def cluster_name(node_ids: set[str]) -> str:
    ...
    for parts in all_parts[1:]:
        common = [c for c, p in zip(common, parts, strict=False) if c == p]
...
def split_dot_file(
    ...
    singleton_groups: dict[str, set[str]] = defaultdict(set)
    multi_components: list[set[str]] = []
    ...
    vprint(
        verbose,
        f"[split] Will generate {len(all_clusters)} file(s) "
    ...
    for cluster_nodes in sorted(all_clusters, key=lambda c: -len(c)):
        if len(cluster_nodes) < min_size:
            vprint(verbose, f"  - skipping {len(cluster_nodes)} node cluster")
            continue
        ...
        dot_content = build_dot(
            name, header_lines, node_lines, edge_lines, concise=concise
        )
        ...
        size_label = (
            f"{len(cluster_nodes)} class{'es' if len(cluster_nodes) != 1 else ''}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-directory", default=".", help="Where to write output files"
    )
    parser.add_argument(
        "--format",
        default="png",
        choices=["png", "svg", "pdf", "jpg", "none"],
        help="Output image format or none",
    )
    parser.add_argument(
        "--min-size",
        type=int,
    ...
    if render_format is not None:
        for dot_file in dot_files:
            out_img = render_dot(
                dot_file, dot_file.parent, fmt=render_format, verbose=args.verbose
            )
